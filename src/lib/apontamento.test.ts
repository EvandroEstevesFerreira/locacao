...
describe("horasDesdeRevisao", () => {
  ...
  it("a leitura DA revisão não entra na conta", () => {
    ...
    expect(horasDesdeRevisao([{ horas: 12, revisao: true }])).toBe(0);
  });
  ...
  it("sem revisão nenhuma, soma o histórico inteiro", () => {
    ...
    expect(horasDesdeRevisao([{ horas: 40, revisao: false }, { horas: 30, revisao: false }])).toBe(70);
  });
  ...
  it("peça sem apontamento devolve null, e não zero", () => {
    ...
    expect(horasDesdeRevisao([])).toBeNull();
  });
...
describe("horasAteRevisao", () => {
  it("conta a partir da última revisão, não do mostrador", () => {
    ...
    expect(horasAteRevisao(240, 250)).toBe(10);
  });
  ...
  it("devolve null quando a peça nunca foi apontada", () => {
    ...
    expect(horasAteRevisao(null, 250)).toBeNull();
  });

  it("máquina recém-revisada tem o intervalo inteiro pela frente", () => {
    expect(horasAteRevisao(0, 250)).toBe(250);
  });
});
